    ...
    <section className="py-24 lg:py-32 bg-slate-50 border-t border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-16 max-w-3xl">
          <div className="text-sm font-semibold tracking-wide text-blue-600 uppercase mb-4">The Sustainable Pipeline</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-8 items-center mb-16">
            ...
            <div className="space-y-3">
              {sources.map((source, idx) => (
                <div key={idx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:border-blue-300 transition-colors">
                  <h4 className="font-bold text-indigo-950 text-sm mb-1">{source.title}</h4>
          ...
          <div className="flex flex-col items-center justify-center">
            {/* Arrows */}
            <div className="flex gap-6 text-indigo-300 font-bold text-xl mb-4">
              <span>↓</span>
              <span>↓</span>
              <span>↓</span>
            </div>
            ...
            {/* Funnel Shape */}
            <div 
              className="bg-gradient-to-b from-blue-500 to-blue-600 flex flex-col justify-between py-12 text-white font-bold text-center tracking-widest shadow-lg"
              ...
              {results.map((res, idx) => (
                <div key={idx} className={`py-6 ${idx !== results.length - 1 ? 'border-b border-slate-200' : ''}`}>
                  <div className="flex items-baseline gap-3 mb-2">
                    <span className="text-4xl font-bold text-indigo-950">{res.metric}</span>
                    <span className="text-xs font-bold text-blue-500 tracking-wider uppercase">{res.stage}</span>
                  </div>
        ...
        {/* Bottom Bar */}
        <div className="bg-blue-500 rounded-xl p-6 text-center shadow-md">
